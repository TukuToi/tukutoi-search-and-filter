<?php
/**
 * The file that defines TukuToi Common functionality
 *
 * @link       https://www.tukutoi.com/
 * @since      1.0.0
 *
 * @package    Tkt_Search_Filter
 * @subpackage Tkt_Search_Filter/common
 */
...
class TKT_Common {
	...
	public function set_general_options_callback( $pre, $middle, $end, $locale ) {
		echo '<p>' . esc_html__( $pre, $locale ) . '<strong>' . esc_html( $middle ) . '</strong>' . esc_html__( $end, $locale ) . '</p>';
	}
	...
	public function set_render_settings_page_content( $active_tab = '', $field = '', $section = '', $locale = '', $label = 'Save Settings', $additional_html = '' ) {
		...
		<div class="wrap">
			...
			<div>
				<div class="main-dashboard-inner">
					...
					if ( ! empty( $additional_html ) ) {
						echo wp_kses( $additional_html, 'post' );}
					?>
			...
			<?php echo wp_kses( $this->feedback_box(), 'post' ); ?>
	...
	public function tkt_common_delete_options_cb( $args ) {
		...
		<input 
			...
			name="<?php echo esc_html( $this->common_name ); ?>[<?php echo esc_attr( $args['label_for'] ); ?>]" 
	...
	public function render_settings_page_content( $active_tab = '' ) {
		...
		$field = $this->common_name;
		$section = $this->common_name;
		$locale = $this->common_name;
		$active_tab = '';

		$this->set_render_settings_page_content( $active_tab, $field, $section, $locale );
